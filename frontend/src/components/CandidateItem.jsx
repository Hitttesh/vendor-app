...
export default function CandidateItem({
  ...
  // Use only proper DB field
  const candidateId = candidate.candidate_uuid;
  const initial = String((candidate.name || "U").charAt(0)).toUpperCase();

  const activeWidthPercent = `${currentIndex * 33.3333333}%`;

  /* ---------- API Status Update (cleaned for new api.js) ---------- */
  async function changeStatus(newStatus) {
    if (!assessmentId || !candidateId) {
      console.warn("Missing assessmentId or candidate_uuid for status update");
      return;
    }

    setLoading(true);
    try {
      // new API signature: (assessmentId, candidateUuid, status)
      await apiUpdateStatus(assessmentId, candidateId, newStatus);
                ...
                <div key={idx} className="w-1/3 flex justify-center">
                  <div
                    className={`w-[14px] h-[14px] rounded-full border-[3px] transition-all ${
                      active
                        ? "bg-green-600 border-green-600 shadow-[0_0_0_6px_rgba(22,163,74,0.06)]"
        ...
        <div className="flex justify-between px-1">
          <p
            className={`flex-1 text-center text-[13px] ${
              currentIndex >= 0 ? "text-gray-900 font-bold" : "text-gray-400"
            }`}
          >
            Invited
          </p>
          <p
            className={`flex-1 text-center text-[13px] ${
              currentIndex >= 1 ? "text-gray-900 font-bold" : "text-gray-400"
            }`}
          >
            Interview
          </p>
          <p
            className={`flex-1 text-center text-[13px] ${
              currentIndex >= 2 ? "text-gray-900 font-bold" : "text-gray-400"
            }`}
          >
            {finalLabel}
          </p>
